4.36.1 – 04.12.2025 13:05
- [Fix] (Testing / Integration Tests) `tests/integration/test_auth_all_services/test_cookie_isolation.py` — **ИСПРАВЛЕНИЕ ТЕСТА**: исправлена проблема с проверкой cookie сессии в тесте `test_no_session_conflict_between_services`. **Проблема:** Тест падал с ошибкой "Cookie сессии не найдено для webhook_server. Cookie: {}" из-за того, что Flask-Session устанавливает cookie в ответе redirect (302), но `requests.Session` с `allow_redirects=True` может не сохранять cookie из промежуточного redirect ответа правильно. **Решение:** Изменена логика проверки cookie для обоих сервисов (webhook_server и docs-proxy): теперь проверяются cookie из истории ответов (`response.history`) при redirect, объединяются cookie из истории и финального ответа, а также проверяются cookie в сессии. Это обеспечивает корректную проверку cookie независимо от того, где именно Flask-Session установил cookie (в redirect ответе или финальном ответе). Улучшена диагностика проблемы: в сообщении об ошибке теперь выводится информация о cookie из всех источников (история redirect, финальный ответ, сессия). **Результат:** Тест успешно проходит на разных типах сервера (production/development), корректно проверяет изоляцию сессий между сервисами.

4.36.0 – 04.12.2025 09:37
- [Fix] (User Cabinet / CSP) `apps/user-cabinet/app.py` — **КРИТИЧНОЕ ИСПРАВЛЕНИЕ**: исправлена проблема с Content Security Policy заголовками, которая блокировала проверку IP клиента. **Проблема:** В настройках БД домены хранятся с протоколом (`https://help.dark-maximus.com`), но код формировал CSP заголовки добавляя `https://` еще раз, что приводило к невалидному CSP: `https://https://help.dark-maximus.com`. Браузер считал CSP невалидным и игнорировал правила, блокируя запросы к внешним API для проверки IP (`ipwho.is` и `api.2ip.io`). JavaScript использовал fallback через серверный эндпоинт, который показывал IP сервера (31.56.27.129) вместо IP пользователя. **Решение:** Изменена логика обработки доменов в двух функциях: 1) `set_csp_headers()` (строки 71-89) — теперь протокол удаляется из доменов ВСЕГДА, независимо от того, пустые они или нет (перенесена логика очистки из блока `else` в общий блок после проверки на пустоту); 2) `health()` (строки 243-260) — применена та же логика очистки доменов. Теперь CSP формируется корректно: `connect-src 'self' https://api.2ip.ru https://api.2ip.io https://ipwho.is https://help.dark-maximus.com https://*.dark-maximus.com`. **Результат:** Браузер разрешает прямые запросы к `ipwho.is` и `api.2ip.io`, JavaScript делает запросы напрямую из браузера пользователя, API видят реальный IP пользователя, в личном кабинете отображается IP пользователя вместо IP сервера.

4.35.0 – 04.12.2025 08:56
- [Optimization] (Deployment / Docker) `install.sh` — **ОПТИМИЗАЦИЯ ВРЕМЕНИ СБОРКИ**: добавлена поддержка кэширования Docker слоев и опциональный параметр `--no-cache` для полной пересборки. **Проблема:** Сборка контейнеров через `install.sh` занимала более 5 минут (365 секунд) из-за использования `--no-cache`, что заставляло Docker пересобирать ВСЕ слои с нуля каждый раз, даже если зависимости не изменились. Основные узкие места: `yarn install` в codex-docs (239.2s), `npm install` в bot (101.4s), Python зависимости (96.1s + 180.1s). **Решение:** 1) Убран `--no-cache` по умолчанию — теперь Docker использует кэш слоев для ускорения сборки; 2) Добавлена обработка опционального параметра `--no-cache` или `--rebuild` через аргументы командной строки (поддержка передачи через URL: `curl ... | bash -s -- domain.com --no-cache`); 3) Включен BuildKit через переменные окружения `DOCKER_BUILDKIT=1` и `COMPOSE_DOCKER_CLI_BUILD=1` для параллельной сборки и улучшенного кэширования; 4) Исправлена логика получения домена из аргументов — теперь корректно пропускаются флаги `--no-cache` и `--rebuild` при извлечении домена. **Результат:** Время сборки значительно сокращено: повторная сборка без изменений — ~30-60s (вместо 365s), повторная сборка с изменениями в коде — ~60-120s (кэш зависимостей, пересборка только измененных слоев). Первая сборка остается ~365s (зависимости нужно установить). Для полной пересборки с нуля можно использовать `--no-cache` или `--rebuild`.

4.34.1 – 04.12.2025 07:27
- [Fix] (Bot / Промокоды / UX) `src/shop_bot/bot/handlers.py` — **КРИТИЧНОЕ ИСПРАВЛЕНИЕ UX**: исправлено вводящее в заблуждение сообщение при повторном переходе по deeplink ссылке с промокодом. **Проблема:** При повторном переходе по deeplink ссылке (например, `https://t.me/maximus_vpn_bot?start=eyJnIjoiZnJpZW5kcyIsInAiOiJGSVZFSCJ9`) бот показывал сообщение "✅ Применен промокод: FIVEH, 💰 На ваш баланс зачислено 500.0 руб.", хотя на самом деле баланс НЕ был зачислен повторно (это уже было исправлено в версии 4.34.0). Это вводило пользователей в заблуждение. **Решение:** 1) Изменена логика разделения промокодов (строки 698-716): при повторном переходе промокод теперь добавляется в список `already_applied_promos` вместо `applied_promos`, если `existing_usage_id` существует; 2) Обновлен текст сообщения для уже примененных промокодов (строки 827 и 6759): изменен с "ℹ️ Промокод уже применён" на "❌ Вы ранее использовали промокод". **Результат:** Теперь при повторном переходе по deeplink ссылке пользователь видит корректное сообщение: "✅ Вы добавлены в группу(ы): friends, ❌ Вы ранее использовали промокод: FIVEH" вместо вводящего в заблуждение "На ваш баланс зачислено". Исправление улучшает пользовательский опыт и предотвращает путаницу.
- [Documentation] (API Reference) `docs/reference/api/promo-codes.md` — Обновлена документация по промокодам: добавлено детальное описание поведения при переходе по deeplink ссылке с разделением на два сценария (первый переход — "Применен промокод, на ваш баланс зачислено", повторный переход — "Вы ранее использовали промокод"), добавлено примечание о том, что бонус начисляется только при первом применении промокода. Обновлена дата последней редакции документа.

4.34.0 – 29.11.2025 13:55
- [Fix] (Bot / Промокоды) `src/shop_bot/bot/handlers.py` — **КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ**: исправлена проблема многократного начисления бонусов при переходе по deeplink ссылке с промокодом. **Проблема:** При каждом переходе по deeplink ссылке с промокодом (например, `https://t.me/bot?start=eyJwIjoiRklWRUgifQ==`) бонус начислялся на баланс пользователя повторно, даже если промокод уже был применен ранее со статусом `'applied'`. Это приводило к неконтролируемому росту баланса пользователей (например, пользователь 6044240344 получил 1470 RUB вместо 500 RUB, пользователь 976124857 получил 1000 RUB вместо 500 RUB). **Решение:** Добавлена проверка `existing_usage_id` перед начислением бонуса в двух местах: 1) В deeplink обработчике (строки 705-714) — бонус начисляется ТОЛЬКО если `existing_usage_id` отсутствует (первое применение промокода), при повторном переходе по ссылке промокод обновляется, но бонус НЕ начисляется повторно; 2) В ручном применении промокода (строки 4657-4664) — аналогичная логика для ручного ввода промокода. Добавлено детальное логирование для отладки: при первом применении логируется "Bonus X RUB applied (first time)", при повторном — "Bonus NOT applied (already applied, existing_usage_id=Y)". Исправление предотвращает многократное начисление бонусов и обеспечивает корректную работу промокодов.
- [Fix] (Database / Промокоды) `src/shop_bot/data_manager/database.py` — **КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ**: исправлена проблема с промокодами без установленных сроков. **Проблема:** Если при создании промокода не установлены "Срок сгорания" (`burn_after`) и "Действителен до" (`valid_until`), промокод переставал работать из-за ошибок парсинга NULL значений в функции `can_user_use_promo_code()` (строки 9411-9446). **Решение:** Добавлена явная проверка на NULL и пустые строки перед парсингом дат: 1) Для `valid_until` (строки 9411-9423) — проверка выполняется ТОЛЬКО если значение не NULL и не пустая строка, при ошибке парсинга промокод работает БЕЗ ОГРАНИЧЕНИЙ по сроку действия; 2) Для `burn_after` (строки 9425-9449) — проверка выполняется ТОЛЬКО если `burn_after_value` и `burn_after_unit` не NULL и не пустые строки, при ошибке парсинга промокод работает БЕЗ ОГРАНИЧЕНИЙ по сроку сгорания. Добавлено логирование ошибок парсинга для диагностики. Теперь промокоды без установленных сроков работают корректно и бессрочно.
- [Testing] (Unit Tests) `tests/unit/test_database/test_promo_code_operations.py` — Добавлены критические unit-тесты для выявления проблем с промокодами: 1) `test_promo_code_bonus_applied_only_once()` — проверяет, что бонус начисляется только один раз при многократном применении промокода (симулирует логику из handlers.py с проверкой `existing_usage_id`); 2) `test_promo_code_without_expiration()` — проверяет, что промокод без установленных сроков (`burn_after` и `valid_until` = NULL) работает корректно и доступен для использования. Оба теста имеют severity CRITICAL и детальное описание в Allure с шагами проверки.
- [Testing] (Integration Tests) `tests/integration/test_deeplink_promo_code_reapplication.py` — Добавлены интеграционные тесты для проверки многократного применения промокода через deeplink: 1) `test_deeplink_promo_code_multiple_clicks()` — полный интеграционный тест, симулирующий первый и повторный переход по deeplink ссылке с проверкой баланса пользователя на каждом этапе, проверяет, что бонус начисляется только при первом переходе, а при повторном баланс не изменяется; 2) `test_used_promo_code_cannot_be_reapplied()` — проверяет, что промокод со статусом `'used'` (использован при покупке) нельзя применить повторно. Тесты имеют детальное описание в Allure с шагами и ожидаемыми результатами.
- [Testing] (E2E Tests) `tests/e2e/test_user_scenarios/test_promo_code_reapplication.py` — Добавлен E2E тест полного пользовательского сценария `test_user_promo_code_deeplink_reapplication()`: проверяет полный цикл от регистрации пользователя и первого перехода по deeplink ссылке до повторного перехода и проверки баланса. Тест имеет severity CRITICAL и детальное описание в Allure с 4 основными шагами: 1) Первый переход по deeplink ссылке (регистрация, применение промокода, начисление бонуса); 2) Проверка баланса после первого перехода (должен быть 500 RUB); 3) Повторный переход по той же ссылке (обновление промокода, НЕ начисление бонуса); 4) Проверка баланса после повторного перехода (должен остаться 500 RUB).
- [Tools] (Ad-hoc Scripts) `tests/ad-hoc/utils/rollback_promo_bonus.py` — Создан скрипт для отката лишних начислений бонусов на продакшне: находит пользователей с многократными начислениями бонусов по промокоду, рассчитывает корректный баланс (первое начисление), откатывает лишние начисления, удаляет дублирующиеся записи в `promo_code_usage`, логирует все изменения. Скрипт поддерживает dry-run режим для просмотра без изменений, фильтрацию по конкретным user_id, работу с продакшн БД через SSH. Использование: `python rollback_promo_bonus.py --dry-run --db-path users.db --promo-code FIVEH --specific-users 6044240344,976124857`.
- [Tools] (Ad-hoc Scripts) `tests/ad-hoc/utils/ROLLBACK_INSTRUCTIONS.md` — Создана подробная инструкция по откату лишних начислений на продакшне: описание проблемы, пошаговые инструкции по выполнению отката (подключение к серверу, проверка dry-run, резервное копирование БД, выполнение отката, проверка результата), инструкции по восстановлению из резервной копии, проверка исправления кода. Инструкция включает примеры команд и ожидаемые результаты для каждого шага.
- [Documentation] (API Reference) `docs/reference/api/promo-codes.md` — Обновлена документация по промокодам: добавлен раздел "Логика начисления бонусов" с детальным описанием алгоритма начисления бонусов (проверка `existing_usage_id`, начисление только при первом применении, примеры кода), добавлен раздел "Правила работы с сроками промокодов" с описанием опциональности полей `valid_until` и `burn_after` (промокоды без установленных сроков работают бессрочно), обновлены разделы "Статус applied" и "Логика работы" с учетом исправлений, добавлен пункт "Бессрочные промокоды" в раздел "Преимущества новой системы".

4.33.0 – 26.11.2025 15:06
- [Fix] (Bot / YooKassa) `src/shop_bot/bot/handlers.py` — **КРИТИЧНОЕ ИСПРАВЛЕНИЕ**: добавлена передача параметра `subscription_link` в функции создания ключей через YooKassa webhook и бесплатные планы. Исправлены три вызова функций: 1) `create_key_with_stats_atomic()` на строке 7069 (создание нового ключа через YooKassa webhook) — добавлен параметр `subscription_link=result.get('subscription_link')`, что обеспечивает сохранение ссылки на подписку в БД при создании ключа после успешной оплаты; 2) `add_new_key()` на строке 4952 (создание бесплатного ключа через callback query) — добавлен параметр `subscription_link=result.get('subscription_link')`; 3) `add_new_key()` на строке 5474 (создание бесплатного ключа через другой callback query) — добавлен параметр `subscription_link=result.get('subscription_link')`. **Проблема:** Хотя `subscription_link` генерировался в `xui_api.create_or_update_key_on_host()` (строка 733-740) и возвращался в `result` (строка 966), он **НЕ ПЕРЕДАВАЛСЯ** в функции сохранения ключа в БД, что приводило к сохранению `NULL` в поле `subscription_link` таблицы `vpn_keys`. Это вызывало проблему отображения "Ссылка на подписку недоступна" в личном кабинете и прочерка `—` в веб-панели для всех новых ключей, созданных через YooKassa webhook или бесплатные планы. **Решение:** Теперь `subscription_link` корректно передается из `result` в функции создания ключа и сохраняется в БД. Исправление обеспечивает корректное отображение ссылки на подписку во всех интерфейсах (личный кабинет, веб-панель, бот) для всех новых ключей. Fallback логика (из версии 4.31.0) продолжает работать для старых ключей, у которых `subscription_link` был `NULL` до этого исправления.

4.32.1 – 26.11.2025 14:55
- [Fix] (Web Panel) `src/shop_bot/webhook_server/app.py` — Добавлен автоматический fallback для генерации `subscription_link` в API endpoint `/api/key/<int:key_id>`: если поле `subscription_link` пустое в БД, но есть `subscription` (sub_id), система автоматически генерирует ссылку на подписку через API 3X-UI и сохраняет её в БД. Теперь поле "Subscription Link" в детальной панели ключа (Key Drawer) в веб-панели будет корректно отображать ссылку на подписку вместо прочерка `—` для ключей, у которых `subscription_link` не был сохранен при создании. Fallback работает аналогично логике в `validate_permanent_token()` и `get_key_by_id()`: проверяет наличие `subscription` и данных ключа (host_name, key_email), вызывает `get_client_subscription_link()` для получения ссылки, сохраняет в БД, логирует все операции. Исправление обеспечивает единообразное отображение `subscription_link` во всех интерфейсах: веб-панель, личный кабинет, бот.

4.32.0 – 26.11.2025 14:34
- [Fix] (Bot) `src/shop_bot/bot/keyboards.py` — Упрощена логика кнопки "⚙️ Настройка": кнопка теперь **всегда** отображается с фиксированным URL `https://help.dark-maximus.com/setup` независимо от настроек `server_environment` (production/development) и `codex_docs_domain`. Удалены все проверки `is_production_server()` и условия для fallback URL. Кнопка "⚙️ Настройка" теперь отображается во всех режимах (production и development), что обеспечивает единообразный пользовательский опыт. Кнопка "🗂️ Личный кабинет" продолжает работать по прежней логике (только в production для режимов cabinet/cabinet_subscription).
- [Testing] (Unit Tests) `tests/unit/test_bot/test_keyboard_domains.py` — Обновлены unit-тесты для кнопок клавиатуры: переименованы и обновлены тесты для кнопки "⚙️ Настройка" с проверкой фиксированного URL `https://help.dark-maximus.com/setup` во всех режимах (production, development, с настройкой codex_docs_domain, с localhost), добавлены тесты для проверки игнорирования настройки `codex_docs_domain` и использования фиксированного URL вместо локального адреса. Все тесты успешно прошли (9 passed). Тесты для кнопки "🗂️ Личный кабинет" остались без изменений (проверяют отображение только в production для режимов cabinet/cabinet_subscription).
- [Documentation] (Reference) `docs/reference/features-subscriptions.md` — Обновлена документация: добавлен новый раздел "Кнопки в клавиатуре управления ключом" с подробным описанием всех кнопок (⚙️ Настройка, 🗂️ Личный кабинет, 🔑 Подписка), их условий отображения и расположения. Добавлены примеры расположения кнопок для разных режимов предоставления (key, both, cabinet, cabinet_subscription). Обновлена дата последнего обновления документа.

4.31.0 – 26.11.2025 14:04
- [Feature] (User Cabinet / Bot) `src/shop_bot/data_manager/database.py` — Добавлен автоматический fallback для генерации `subscription_link` на лету: если поле `subscription_link` пустое в БД, но есть `subscription` (sub_id), система автоматически генерирует ссылку на подписку через API 3X-UI и сохраняет её в БД для будущих запросов. Исправление применено в двух функциях: `validate_permanent_token()` (используется личным кабинетом для отображения ссылки на подписку на вкладке "Подключение") и `get_key_by_id()` (используется ботом для отображения кнопки "Подписка"). Теперь ссылка на подписку будет корректно отображаться в личном кабинете даже для ключей, созданных до внедрения функциональности `subscription_link`, или для ключей, у которых `subscription_link` не был сохранен при создании из-за ошибок получения `subURI` из панели 3X-UI. Fallback работает следующим образом: 1) Проверяет наличие `subscription` (sub_id) и данных ключа (host_name, key_email); 2) Вызывает функцию `get_client_subscription_link()` для получения ссылки через API 3X-UI; 3) Сохраняет полученную ссылку в БД для оптимизации будущих запросов; 4) Возвращает ссылку в `token_data` или `key_dict` для отображения. Все ошибки логируются с детальной информацией для диагностики. Исправление решает проблему отображения "Ссылка на подписку недоступна" в личном кабинете при наличии рабочей ссылки на подписку в 3X-UI.

4.30.8 – 26.11.2025 12:32
- [Fix] (Testing) `tests/unit/test_database/test_domain_settings.py` — Исправлены тесты `test_get_global_domain_fallback_to_localhost_in_development` и `test_get_global_domain_returns_none_in_production`: улучшена логика очистки настроек доменов — используется прямое удаление через SQL без предварительной вставки пустых строк для избежания race condition, добавлен PRAGMA busy_timeout для предотвращения блокировок SQLite, улучшена синхронизация соединений с увеличенной задержкой после удаления (0.2 сек), добавлена явная проверка сохранения настройки `server_environment` через `get_server_environment()` и функции проверки окружения (`is_development_server()`, `is_production_server()`), добавлена дополнительная диагностическая информация через `allure.attach` для отладки (значения из БД до и после очистки, состояние окружения, результаты функций). Тесты теперь корректно работают на разных типах серверов (production и development), так как явно устанавливают и проверяют окружение независимо от реального типа сервера.

4.30.7 – 25.11.2025 13:47
- [Fix] (SSL/Network) `src/shop_bot/bot_controller.py` — Улучшена настройка SSL контекста для подключения к Telegram API: создан кастомный класс `CustomAiohttpSession`, который явно настраивает SSL контекст с использованием `certifi.where()` и дополнительными параметрами безопасности (`check_hostname=True`, `verify_mode=CERT_REQUIRED`), что помогает предотвратить ошибки типа "SSL record layer failure" при временных проблемах с сетью. Функция `_create_telegram_session()` теперь использует `CustomAiohttpSession` вместо стандартного `AiohttpSession` для большей надежности SSL соединений.
- [Fix] (SSL/Network) `src/shop_bot/utils/error_handler.py` — Улучшена обработка SSL ошибок в функции `_handle_network_error()`: добавлено детальное логирование для различных типов SSL ошибок (record layer failure, certificate errors, handshake errors), добавлена информация о том, что aiogram автоматически повторяет попытки при временных ошибках, добавлены рекомендации по диагностике и решению проблем (проверка интернет-соединения, настройки файрвола, актуальность SSL сертификатов, системное время). Ошибки "SSL record layer failure" теперь логируются как предупреждения с подробными инструкциями, а критические ошибки сертификатов логируются как ошибки с рекомендациями по исправлению.

4.30.6 – 25.11.2025 03:34
- [Fix] (TON Explorer) `src/shop_bot/bot/handlers.py`, `src/shop_bot/webhook_server/templates/transactions.html`, `src/shop_bot/webhook_server/templates/check_payment.html`, `src/shop_bot/webhook_server/static/js/script.js` — Исправлена ссылка на TON Explorer: изменен URL с `tonscan.org` на `tonviewer.com` во всех местах (функция `get_ton_transaction_url()`, шаблоны транзакций, страница проверки платежа, JavaScript код). Добавлена проверка формата хеша транзакции в функции `get_ton_transaction_url()` для корректной обработки hex-хеша (64 символа) и BOC (base64). Исправление обеспечивает корректную работу ссылок на просмотр транзакций в TON Explorer.
- [Fix] (Timezone) `src/shop_bot/data_manager/database.py` — Исправлена проблема с часовым поясом на боевом сервере: добавлена колонка `timezone` в таблицу `users` (миграция уже была в коде, но не была выполнена на боевом сервере), включен feature flag `feature_timezone_enabled = 1` в настройках БД, добавлена настройка `admin_timezone = 'Europe/Moscow'` в таблицу `bot_settings`. Исправление обеспечивает корректную работу часовых поясов на боевом сервере: пользователи теперь могут устанавливать свой часовой пояс, и даты ключей будут отображаться в выбранном часовом поясе вместо UTC+3.

4.30.5 – 25.11.2025 02:49
- [Fix] (Bot) `src/shop_bot/config.py` — Исправлена проблема с переменной `{auto_renewal_status}` в шаблонах информации о ключе: добавлена переменная `auto_renewal_status` в словарь `template_variables` в функции `get_key_info_text`, что позволяет использовать переменную `{auto_renewal_status}` в шаблонах БД для отображения статуса автопродления ("Включено 🟢" или "Отключено 🔴").
- [Fix] (Database) `src/shop_bot/data_manager/database.py` — Добавлена переменная `"auto_renewal_status"` в списки разрешённых переменных для всех шаблонов `key_info_*` (key_info_key, key_info_subscription, key_info_both, key_info_cabinet, key_info_cabinet_subscription), что позволяет использовать переменную в шаблонах через веб-панель без ошибок валидации.
- [Testing] (Unit Tests) `tests/unit/test_utils/test_message_templates.py` — Обновлены тесты для функции `get_key_info_text`: добавлена проверка наличия информации об автопродлении в сгенерированном тексте, добавлен параметр `key_auto_renewal_enabled` во все вызовы функции для корректного тестирования.
- [Testing] (Unit Tests) `tests/unit/test_utils/test_template_validation.py` — Обновлена документация в тесте валидации шаблонов: добавлена переменная `{auto_renewal_status}` в список валидных переменных для шаблонов.
- [Documentation] (Testing) `docs/guides/testing/template-validation.md` — Обновлена документация по валидации шаблонов: добавлена переменная `{auto_renewal_status}` в список валидных переменных с описанием формата значений ("Включено 🟢" или "Отключено 🔴"), обновлена дата документа.

4.30.4 – 25.11.2025 02:27
- [Fix] (Bot) `src/shop_bot/bot/keyboards.py` — Исправлена кнопка "Настройка": для localhost используется fallback на рабочий URL `https://help.dark-maximus.com/setup` вместо попыток использовать localhost (Telegram не принимает localhost URLs ни в `url`, ни в `web_app` кнопках). Для production доменов используется нормализованный URL из настроек БД с преобразованием `http://` в `https://`. Кнопка всегда добавляется, как раньше работало. Исправлена кнопка "Личный кабинет": изменена с `web_app` на обычную ссылку `url` для корректной работы на всех окружениях. Добавлена обработка ошибок с детальным логированием (`exc_info=True`) для диагностики проблем при создании кнопок.
- [Fix] (Bot) `src/shop_bot/bot/handlers.py` — Добавлено детальное логирование ошибок (`exc_info=True`) в функции `show_key_handler` для полного traceback при ошибках получения данных ключа.
- [Fix] (User Cabinet) `apps/user-cabinet/app.py` — Исправлены CSP заголовки для личного кабинета: изменен `frame-ancestors` для разрешения Telegram доменов (`https://t.me`, `https://web.telegram.org`), что позволяет встраивать личный кабинет в Telegram Web App. Для development режима в `frame-ancestors` разрешены любые домены (`*`), в `frame-src` разрешены любые домены (`http:`, `https:`) для работы с localhost и внешними провайдерами подписки. Для production режима в `frame-ancestors` разрешены все поддомены главного домена из настроек БД (`https://*.{main_domain}`) и Telegram домены, в `frame-src` разрешены все поддомены главного домена (`https://*.{main_domain}`) и любые HTTPS домены (`https:`) для внешних провайдеров подписки. Изменения применены как в основной функции `set_csp_headers()`, так и в health endpoint. Это исправляет ошибку ERR_BLOCKED_BY_CSP при загрузке iframe с инструкциями (setup-iframe) и подпиской (subscription-iframe) на localhost и production.

4.30.3 – 25.11.2025 02:00
- [Fix] (Bot) `src/shop_bot/bot/keyboards.py` — Исправлена кнопка "Настройка" на localhost: добавлена проверка на localhost в `setup_url` для определения типа кнопки, для localhost используется обычная кнопка `url` вместо `web_app` (Telegram Web App не может открыть `https://localhost:50002/setup` без SSL сертификата), для production используется `web_app` с HTTPS как раньше. Исправлена кнопка "Личный кабинет": изменена с `web_app` на обычную ссылку `url` для корректной работы на всех окружениях.
- [Fix] (User Cabinet) `apps/user-cabinet/app.py` — Исправлены CSP заголовки для личного кабинета: изменен `frame-ancestors` для разрешения Telegram доменов (`https://t.me`, `https://web.telegram.org`), что позволяет встраивать личный кабинет в Telegram Web App. Для development режима в `frame-src` разрешены любые домены (`http:`, `https:`) для работы с localhost и внешними провайдерами подписки. Для production режима в `frame-src` разрешены все поддомены главного домена из настроек БД (`https://*.{main_domain}`) и любые HTTPS домены (`https:`) для внешних провайдеров подписки. Изменения применены как в основной функции `set_csp_headers()`, так и в health endpoint. Это исправляет ошибку ERR_BLOCKED_BY_CSP при загрузке iframe с инструкциями (setup-iframe) и подпиской (subscription-iframe) на localhost и production.
