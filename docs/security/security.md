...
- **Уникальные имена cookie** для каждого сервиса (предотвращение конфликтов сессий)

### Изоляция сессий между сервисами

Все сервисы с авторизацией используют уникальные имена cookie для предотвращения конфликтов при использовании одинакового `FLASK_SECRET_KEY`:

- **Веб-панель** (порт 50000): `panel_session`
- **Docs Proxy** (порт 50001): `docs_session`
- **Allure Homepage** (порт 50005): `allure_session`
- **Личный кабинет** (порт 50003): `cabinet_session`

Это гарантирует, что сессии разных сервисов не конфликтуют друг с другом, даже при работе на одном домене (localhost) с разными портами.
...
# Уникальное имя cookie для каждого сервиса (предотвращение конфликтов)
app.config['SESSION_COOKIE_NAME'] = 'panel_session'  # Для веб-панели
# Для других сервисов: 'docs_session', 'allure_session', 'cabinet_session'
```
...
### Конфликт cookie между сервисами

**Проблема:** При использовании одинакового `FLASK_SECRET_KEY` и дефолтного имени cookie `session` между разными сервисами может возникать конфликт, приводящий к потере сессии при переходах между сервисами.

**Решение:** Каждый сервис использует уникальное имя cookie через параметр `cookie_name` в функции `init_flask_auth()`:

```python
# Для веб-панели (дефолт)
init_flask_auth(app, session_dir='/app/sessions')  # cookie_name='panel_session'
...
# Для личного кабинета (свой Flask app)
app.config['SESSION_COOKIE_NAME'] = 'cabinet_session'
```

### Проверка изоляции сессий

Автотесты проверяют, что каждый сервис использует уникальное имя cookie и сессии не конфликтуют между сервисами. См. `tests/unit/test_webhook_server/test_auth_utils.py` и `tests/integration/test_auth_all_services/`.

---

*Документация создана: 29.09.2025*
*Последнее обновление: 20.11.2025*
*Версия проекта: 4.24.3*
