...
> **Дата последней редакции:** 17.11.2025
...
```
project/
├── allure-results/         # JSON результаты тестов (генерируются автоматически)
...
**Важно:** Содержимое `allure-results/` и `allure-report/` игнорируется в `.gitignore`, но директории сохраняются через `.gitkeep`.
...
Категории применяются автоматически при генерации отчетов:
...
Allure Service автоматически применяет категории при генерации отчетов.
