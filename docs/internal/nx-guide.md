...
> Последнее обновление: 13.11.2025 13:06
...
├── user-docs/            # Docsify контейнер пользовательской Wiki
...
## Сохранение логов бота в каталоге `logs/`
```powershell
$timestamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$logPath = Join-Path (Resolve-Path ./logs) "bot-nx-$timestamp.log"
npx --yes nx logs bot --tail=500 --no-color 2>&1 | Out-File -FilePath $logPath -Encoding UTF8
```

> Команда выше гарантирует, что сохранённый файл окажется в репозитории, а не во временной директории Windows.
...
# docs/internal/wiki/public -> apps/user-docs/
