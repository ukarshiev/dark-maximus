...
- **Для сохранения логов и диагностических дампов:**
  - Всегда направляй вывод команд (`npx nx logs`, `docker compose logs`, `Get-Content` и др.) в файлы внутри `logs/`.
  - В PowerShell используй `Join-Path (Resolve-Path ./logs) "<имя_файла>.log"` вместо временных директорий (`$env:TEMP`).
