...
> Дата последней редакции: 19.11.2025 12:41  
> Версия: 2.71.1

## 🚀 Быстрая установка

### Быстрые команды установки {#_1}

Для быстрого развертывания на боевом сервере используйте следующие команды:

**1. Установка основного проекта:**
```bash
curl -sSL https://raw.githubusercontent.com/ukarshiev/dark-maximus/main/install.sh | sudo bash -s -- domain.com
```

**2. Установка SSL:**
```bash
curl -sSL https://raw.githubusercontent.com/ukarshiev/dark-maximus/main/ssl-install.sh | sudo bash -s -- domain.com
```

**3. Установка автотестов (опционально):**
```bash
curl -sSL https://raw.githubusercontent.com/ukarshiev/dark-maximus/main/install-autotest.sh | sudo bash
```

> **Примечание:** Замените `domain.com` на ваш домен во всех командах.
...
2. **Настройка SSL и HTTPS** — `ssl-install.sh`
...
**1. Запустите скрипт настройки SSL:**

```bash
curl -sSL https://raw.githubusercontent.com/ukarshiev/dark-maximus/main/ssl-install.sh | sudo bash -s -- domain.com
```

> **Важно:** Замените `domain.com` на ваш основной домен. Скрипт автоматически определит папку проекта и настроит все необходимые поддомены.

**2. Следуйте инструкциям:**

Скрипт автоматически:
- Проверит DNS записи для всех поддоменов
- Установит Certbot и необходимые зависимости
- Выпустит Let's Encrypt SSL сертификаты для всех доменов
...
### Этап 3: Установка автотестов (опционально)

Для установки системы автоматизированного тестирования используйте следующий скрипт:

**1. Запустите скрипт установки автотестов:**

```bash
curl -sSL https://raw.githubusercontent.com/ukarshiev/dark-maximus/main/install-autotest.sh | sudo bash
```

> **Примечание:** Скрипт автоматически определит папку проекта и установит все необходимые сервисы.

**2. Следуйте инструкциям:**

Скрипт автоматически:
- Проверит наличие необходимых файлов
- Создаст директории для результатов Allure
- Добавит сервисы автотестов в `docker-compose.yml`
- Соберет образы для автотестов
- Запустит сервисы: `autotest`, `allure-service`, `allure-homepage`

**3. Проверка установки:**

После установки доступны следующие сервисы:

- **Allure Homepage** (локально): `http://localhost:50005`
  - Веб-интерфейс отчетов: `http://localhost:50005/allure-docker-service/projects/default/reports/latest/index.html`
  - API документация (Swagger UI): `http://localhost:50005`
  - API проектов: `http://localhost:50005/allure-docker-service/projects`

- **Контейнер автотестов**: `dark-maximus-autotest`
  - Для запуска тестов: `docker compose exec autotest pytest`
...
# Просмотреть логи
docker compose logs -f autotest allure-service allure-homepage
```

**5. Внешний доступ:**

Для внешнего доступа к отчетам Allure через HTTPS настройте SSL (см. [Этап 2: Настройка SSL и HTTPS](#этап-2-настройка-ssl-и-https)). После настройки SSL будет доступно:
- `https://allure.domain.com/allure-docker-service/projects/default/reports/latest/index.html`

**6. Дополнительная информация:**

Подробнее о системе тестирования см.:
- [Документация по тестированию](../../guides/testing/) — руководство по работе с тестами
- [Allure Framework](../../guides/testing/allure-reporting.md) — описание системы отчетов
...
4. Прямые порты (для отладки):
   - Бот: http://localhost:50000
   - Документация: http://localhost:50001
   - Админ-документация: http://localhost:50002

🔧 Следующие шаги:
1. Настройте DNS A-записи для всех доменов на IP этого сервера
2. Для настройки SSL запустите: `curl -sSL https://raw.githubusercontent.com/ukarshiev/dark-maximus/main/ssl-install.sh | sudo bash -s -- domain.com`
...
Введите Shop ID и Секретный ключ из вашего личного кабинета YooKassa. Сохраните настройки.
...
В меню с вашим приложением включите вебхуки на:
```
...
- **Порты:** 80, 443, 50000 (открыты в файрволе)
...
sudo certbot --nginx -d your-domain.com -d docs.your-domain.com -d help.your-domain.com
...
- Убедитесь, что порты 80, 443 и 50000 свободны.
...
- Проверьте, что порты 50001 и 50002 свободны.
...
sudo netstat -tulpn | grep -E '50000|50001|50002'

# Освобождение портов (если нужно)
sudo lsof -ti:50000 | xargs kill -9
sudo lsof -ti:50001 | xargs kill -9
sudo lsof -ti:50002 | xargs kill -9
```
...
1. Проверьте [FAQ](../../guides/user/faq.md)
2. Посмотрите [Оплату и баланс](../../guides/user/payment.md)
3. Изучите [Production Checklist](../../guides/deployment/deployment-checklist.md)
4. Проверьте [Security Checklist](../../security/security-checklist.md)
5. Посмотрите [Docker Architecture](../../architecture/docker-architecture.md)
6. Создайте Issue в репозитории
7. Обратитесь в Telegram: [@ukarshiev](https://t.me/ukarshiev)
...
- [Docker Architecture](../../architecture/docker-architecture.md) — подробное описание Docker-архитектуры
- [Production Checklist](../../guides/deployment/deployment-checklist.md) — чек-лист для production
- [Security Checklist](../../security/security-checklist.md) — чек-лист безопасности
- [Quickstart](../admin-quickstart.md) — быстрый старт для администраторов
- [Руководство по панели](../../guides/admin/admin-panel-guide.md) — подробности интерфейса
