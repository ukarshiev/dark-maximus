...
- **Режим журналирования**: DELETE (стандартный режим)
...
### Текущий режим: DELETE
Проект использует стандартный режим журналирования DELETE вместо WAL (Write-Ahead Logging).

### Почему DELETE вместо WAL
- **Гарантированная персистентность**: Все изменения записываются напрямую в основную базу данных
- **Простота в Docker**: WAL файлы (`users.db-wal`, `users.db-shm`) не монтируются с хоста и теряются при рестарте контейнера
- **Достаточная производительность**: Для одного процесса и текущей нагрузки DELETE режим обеспечивает необходимую производительность
- **Упрощение миграции**: DELETE режим ближе к PostgreSQL, что упростит будущую миграцию

### Как работает DELETE режим
- Все изменения (INSERT, UPDATE, DELETE) записываются напрямую в `users.db`
- Нет дополнительных файлов журнала
- Изменения видны сразу после `commit()`
- При рестарте контейнера все данные сохраняются

### Проверка режима журналирования
```bash
# Проверка режима журналирования
sqlite3 users.db "PRAGMA journal_mode;"
# Ожидаемый результат: delete

# Проверка файлов базы данных
ls -la users.db*
# Должен быть только users.db (без users.db-wal и users.db-shm)
```
