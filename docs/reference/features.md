...
- **Автопродление**: Гибкая система автоматического продления ключей:
  - **Глобальное автопродление**: Общая настройка автопродления для всех ключей пользователя
  - **Индивидуальное автопродление**: Управление автопродлением для каждого ключа отдельно
  - **Гибкое управление**: Возможность включить/отключить автопродление для конкретного ключа независимо от глобальной настройки
  - **Автоматическое списание**: Продление происходит автоматически с баланса при достаточных средствах
  - **Визуальная индикация**: Статус автопродления отображается в информации о ключе
...
- **Привязка к боту магазина**: Промокоды работают только в боте магазина (shop bot)
...
- **Статистика**: Отображение количества использований и статуса активности
  ...
  - **Автоматическая конвертация триального ключа в боевой при продлении с оплатой**: При продлении триального ключа с оплатой реальными деньгами (price > 0) ключ автоматически конвертируется в боевой
    - Автоматическая установка `is_trial = 0` при продлении с оплатой
    - Автоматическое обновление статуса с `trial-active`/`trial-ended` на `pay-active`/`pay-ended`
    - Работает как для ручного продления, так и для автопродления
    - Гарантирует, что триальные ключи остаются триальными только на период пробного использования (3 дня)
