...
def get_key_by_id(key_id: int):
    ...
    try:
        ...
        with _get_db_connection() as conn:
            ...
            if not key_dict.get('subscription_link') and key_dict.get('subscription'):
                try:
                    import asyncio
                    from shop_bot.modules.xui_api import get_client_subscription_link
                    
                    host_name = key_dict.get('host_name')
                    key_email = key_dict.get('key_email')
                    
                    if host_name and key_email:
                        logging.info(
                            f"Subscription link is empty for key_id={key_id}, attempting to generate from "
                            f"subscription={key_dict.get('subscription')}, host={host_name}, email={key_email}"
                        )
                        ...
                        if subscription_link:
                            key_dict['subscription_link'] = subscription_link
                            logging.info(
                                f"Successfully generated subscription_link for key_id={key_id}: {subscription_link}"
                            )
                            ...
                            try:
                                ...
                                conn.commit()
                                logging.info(f"Saved subscription_link to database for key_id={key_id}")
                            except sqlite3.Error as update_error:
                                logging.warning(
                                    f"Failed to save subscription_link to database for key_id={key_id}: {update_error}"
                                )
                        else:
                            logging.warning(
                                f"Failed to generate subscription_link for key_id={key_id} via API"
                            )
                    else:
                        logging.warning(
                            f"Cannot generate subscription_link for key_id={key_id}: "
                            f"host_name={host_name}, key_email={key_email}"
                        )
                except Exception as fallback_error:
                    logging.error(
                        f"Error during subscription_link fallback generation for key_id={key_id}: {fallback_error}",
                        exc_info=True
                    )
...
def validate_permanent_token(token: str) -> dict | None:
    ...
    try:
        with sqlite3.connect(DB_FILE, timeout=30) as conn:
            ...
            # Получаем дополнительные поля для fallback генерации subscription_link
            cursor.execute('''
                SELECT ut.token, ut.user_id, ut.key_id, ut.created_at, ut.last_used_at, ut.access_count,
                       k.expiry_date, k.status, k.enabled, k.subscription_link, k.subscription, k.host_name, k.key_email
                FROM user_tokens ut
            ...
            if token_data.get('expiry_date') is None:
                ...
                # FALLBACK: Если subscription_link пустой, но есть subscription (sub_id), генерируем ссылку на лету
                if not token_data.get('subscription_link') and token_data.get('subscription'):
                    try:
                        import asyncio
                        from shop_bot.modules.xui_api import get_client_subscription_link
                        
                        host_name = token_data.get('host_name')
                        key_email = token_data.get('key_email')
                        
                        if host_name and key_email:
                            logging.info(
                                f"Subscription link is empty for key_id={key_id}, attempting to generate from "
                                f"subscription={token_data.get('subscription')}, host={host_name}, email={key_email}"
                            )
                            ...
                            if subscription_link:
                                token_data['subscription_link'] = subscription_link
                                logging.info(
                                    f"Successfully generated subscription_link for key_id={key_id}: {subscription_link}"
                                )
